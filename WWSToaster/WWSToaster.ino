...
const int SIM_RX_PIN = 11;
const int SIM_TX_PIN = 10;
...
void connect(char *ClientIdentifier, char UserNameFlag, char PasswordFlag, char *UserName, char *Password, char CleanSession, char WillFlag, char WillQoS, char WillRetain, char *WillTopic, char *WillMessage)
{
  ...
  char* dio;
  strcat(dio, sendLength(localLength));
  strcat(dio, sendUTFString(ProtocolName));
  strcat(dio, ""+(char(4)));
  strcat(dio, ""+(char(UserNameFlag * 128 + PasswordFlag * 64 + WillRetain * 32 + WillQoS * 8 + WillFlag * 4 + CleanSession * 2)));
  strcat(dio, ""+(char(60 / 256)));
  strcat(dio, ""+(char(60 % 256)));
  strcat(dio, sendUTFString(ClientIdentifier));
  if (WillFlag != 0)
  {
    strcat(dio, sendUTFString(WillTopic));
    strcat(dio, sendUTFString(WillMessage));
  }
  if (UserNameFlag != 0)
  {
    strcat(dio, sendUTFString(UserName));
    if (PasswordFlag != 0)
    {
      strcat(dio, sendUTFString(Password));
    }
  }
  publish(0,0,0,1,"hello", "DIO");
}
